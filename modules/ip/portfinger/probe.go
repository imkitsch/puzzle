...
func (N *NmapProbe) parseProbesFromContent(content *string) {
	...
	if len(lines) == 0 {
		gologger.Fatalf("[-] [端口扫描] nmap指纹库数据为空\n")
	}
	...
	for _, line := range lines {
		...
		if c > 1 {
			gologger.Debugf("[-] [端口扫描] nmap指纹库格式错误，只能有一个Exclude标识符，且该标识符应该在首行")
		}
	}
	...
	if !(strings.HasPrefix(l, "Exclude ") || strings.HasPrefix(l, "Probe ")) {
		gologger.Debugf("[-] [端口扫描] nmap指纹库解析失败，首行应该由Probe或Exclude标识符开始")
	}
...
func (p *Probe) parseProbeInfo(probeStr string) {
	...
	if !(proto == "TCP " || proto == "UDP ") {
		gologger.Debugf("[-] [端口扫描] 解析nmap指纹库失败，protocol字段必须为TCP或UDP")
	}
	if len(other) == 0 {
		gologger.Debugf("[-] [端口扫描] 解析nmap指纹库失败，探测名称描述字段名为空")
	}
	...
	if len(dataList) > 0 {
		...
		if err != nil {
			gologger.Debugf("[-] [端口扫描] nmap指纹库编码发送包失败[%s]:  %s\n", dataList[0], err)
		} else {
			p.Data = dataByte
		}
	}
