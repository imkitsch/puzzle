...
func (N *NmapProbe) grabResponse(addr string, Indexes, SocketTimeout int) ([]byte, error) {
	...
	gologger.Debugf("Index.Data: %v", string(N.Probes[Indexes].Data))
...
func (N *NmapProbe) ScanWithProbe(host, port string, SocketTimeout int) *Result {
	...
	if len(defaultProbe) > 0 {
		...
		for resp := range chanResult {
			gologger.Debugf("[+] 默认端口指纹获取成功:%s:%s %s", host, port, resp.ServiceName)
			return resp
		}
	}
	...
	if len(oneProbe) > 0 {
		...
		close(chanResult)
		for resp := range chanResult {
			gologger.Debugf("[+] 级别1指纹获取成功:%s:%s %s\n", host, port, resp.ServiceName)
			return resp
		}
	}
	...
	if len(sixProbe) > 0 {
		...
		close(chanResult)
		for resp := range chanResult {
			gologger.Debugf("[+] 级别<6指纹获取成功:%s:%s %s", host, port, resp.ServiceName)
			return resp
		}
	}
	...
	if len(nineProbe) > 0 {
		...
		close(chanResult)
		for resp := range chanResult {
			gologger.Debugf("[+] 级别<9指纹获取成功:%s:%s %s", host, port, resp.ServiceName)
			return resp
		}
	}

	// 若未识别出指纹，则按照默认端口对应的指纹返回
	gologger.Debugf("[-] 未知服务:%s:%s", host, port)
	Resulttmp := Result{}
...
func (N *NmapProbe) ResultSocket(address string, Indexes, SocketTimeout int, ResultChan chan *Result) {
	//gologger.Debugf("调用ResultSocket函数 %v %d \n", address, Indexes)
	responeData, err := N.grabResponse(address, Indexes, SocketTimeout)
	if err != nil { // 端口发送指纹失败
		return
	}
	//gologger.Debugf("socket 返回 responseData %v \n", string(responeData))
