...
func asnCheck(ip string) bool {
	IP := net.ParseIP(ip)
	for _, cdn := range CDNs {
		_, tmp, err := net.ParseCIDR(cdn)
		if err != nil {
			gologger.Fatalf("查询cdn失败:%s", err.Error())
		}
		if tmp.Contains(IP) {
			return true
		}
	...
	response, err := reader.ASN(IP)
	if err != nil {
		gologger.Fatalf("获取ASN失败:%s", err.Error())
	}
	for _, ASN := range ASNS {
		IntASN, _ := strconv.Atoi(ASN)
		if int(response.AutonomousSystemNumber) == IntASN {
			return true
		}
